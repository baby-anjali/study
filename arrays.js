...
export const product = (array) =>
  array.reduce((init, element) => init * element, 1);

// 7. Write a function to remove all falsy values from an array:
// `[0, 1, false, 2, '', 3, null, 4, undefined]`.

export const filterTruthy = (array) => array.filter((element) => element);
...
export const filterPrimes = (array) =>
  array.filter((element) => isPrime(element));
...
export const squareRoots = (array) => array.map((number) => Math.sqrt(number));
